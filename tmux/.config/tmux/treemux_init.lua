-- treemux sidebar 用 neovim init file (カスタム版)
-- オリジナル: ~/.tmux/plugins/treemux/configs/treemux_init.lua
-- カスタマイズ箇所は [CUSTOM] コメントで明示
--   1. file_open_requested: edit → tabnew (既存 vim に新タブで開く)
--   2. file_open_requested: nvim → exec nvim (vim 終了時に pane を閉じる)
        ...
        event_handlers = {
          {
            ...
            handler = function(args)
              ...
              -- [CUSTOM] os.execute を一時ラップ: nvim → exec nvim に変換
              -- exec でシェルを置き換えるため、vim 終了時に pane も閉じる
              local orig_execute = os.execute
              os.execute = function(cmd)
                if cmd:match("send%-keys .+ nvim$") then
                  cmd = cmd:gsub(" nvim$", " 'exec nvim'")
                end
                return orig_execute(cmd)
              end

              -- [CUSTOM] 既存 vim がある場合は新しいタブで開く (edit だとバッファが上書きされる)
              nt_remote.remote_nvim_open(nil, "tabnew", args.path, tmux_opts)
